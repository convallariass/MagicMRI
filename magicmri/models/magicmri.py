...
# Licensed under the MIT License; see licenses/BAAI-MIT.txt.
...
class Block(nn.Module):
    ...
    def __init__(
        ...
        if use_residual_block:
            raise RuntimeError("Residual training variants are not included in this inference runtime")
...
class MagicMRI(nn.Module):
    """Masked image model with a visual source-target exemplar context."""
    def __init__(
             self,
             img_size=224,
             patch_size=16,
    ...
    def patchify(self, imgs):
        """Convert [N, 3, 2H, H] images to flattened patch targets."""
        p = self.patch_size
    ...
    def unpatchify(self, x):
        """Convert flattened patch predictions to [N, 3, 2H, H] images."""
        p = self.patch_size
    ...
    def forward_loss(self, pred, targets, mask, valid):
        """Compute the production masked SmoothL1 reconstruction objective."""
        mask = mask[:, :, None].repeat(1, 1, self.patch_size**2 * 3)
        mask = self.unpatchify(mask)

        mean = torch.tensor([0.485, 0.456, 0.406], device=targets.device)[None, :, None, None]
        std = torch.tensor([0.229, 0.224, 0.225], device=targets.device)[None, :, None, None]
        ignore = ((targets * std + mean) * (1 - mask.float())).sum((1, 2, 3)) < 300
        if ignore.any():
            valid[ignore] = 0.0
        mask = mask * valid

        if self.loss_func == "l1l2":
            loss = ((pred - targets).abs() + (pred - targets) ** 2) * 0.5
        elif self.loss_func == "l1":
            loss = (pred - targets).abs()
        elif self.loss_func == "l2":
            loss = (pred - targets) ** 2
        elif self.loss_func == "smoothl1":
            loss = F.smooth_l1_loss(pred, targets, reduction="none", beta=0.01)
        else:
            raise ValueError(f"Unsupported loss: {self.loss_func}")
        return (loss * mask).sum() / (mask.sum() + 1e-2)

    def forward(self, images, targets, bool_masked_pos=None, valid=None):
        if bool_masked_pos is None:
            bool_masked_pos = torch.zeros(
                (images.shape[0], self.patch_embed.num_patches),
                dtype=torch.bool,
        ...
        latent = self.forward_encoder(images, targets, bool_masked_pos)
        prediction = self.forward_decoder(latent)
        loss = self.forward_loss(prediction, targets, bool_masked_pos, valid)
        return loss, self.patchify(prediction), bool_masked_pos

def magicmri_vit_large_patch16_input896x448(**kwargs):
    model = MagicMRI(
